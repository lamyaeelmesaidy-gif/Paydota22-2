...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      if (existingAdmins.length > 0) {
        return res.status(409).json({ 
          message: "Admin user already exists. Admin setup is only allowed once." 
        });
      }
  ...
  // Promote existing user to admin (requires existing admin)
  app.post("/api/admin/promote", requireAuth, async (req: any, res) => {
    try {
