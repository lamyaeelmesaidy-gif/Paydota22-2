...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Card Deposit via Flutterwave - creates a payment link and redirects user
  app.post('/api/deposit/card', requireAuth, async (req: any, res) => {
    try {
      const userId = req.session?.userId;
      if (!userId) {
        return res.status(401).json({ message: 'Unauthorized' });
      ...
      console.log(`💳 Creating card deposit for user ${userId}: ${amount} ${currency}`);
      console.log(`   Redirect URL: ${redirectUrl}`);

      // Create Flutterwave payment link
      const paymentLinkResponse = await flutterwaveService.createPaymentLink({
        txRef,
        amount: amount.toString(),
        currency,
        redirectUrl,
        paymentOptions: 'card',
        customer: {
          email: user.email || `user${userId}@probrandify.com`,
          name: user.fullName || user.username || 'Customer',
          phonenumber: user.phone || undefined,
        },
        customizations: {
          title: 'PROBRANDIFY Wallet Deposit',
          description: `Deposit ${amount} ${currency} to your wallet`,
          logo: 'https://via.placeholder.com/100x100.png?text=PB',
        },
        metadata: {
          userId,
          type: 'wallet_deposit',
      ...
      await storage.createPaymentLink({
        ...
        description: `Card deposit of ${amount} ${currency}`,
        amount: amount.toString(),
        currency,
        customerEmail: user.email || `user${userId}@probrandify.com`,
        customerName: user.fullName || user.username || 'Customer',
        customerPhone: user.phone || undefined,
        flutterwaveLink: paymentLinkResponse.data.link,
        status: 'active',
      });

      console.log(`✅ Card deposit payment link created: ${paymentLinkResponse.data.link}`);

      res.json({
        success: true,
        paymentUrl: paymentLinkResponse.data.link,
        txRef,
        amount: amount.toString(),
        currency,
      });
    } catch (error: any) {
      console.error('Error creating card deposit:', error);
      res.status(500).json({
