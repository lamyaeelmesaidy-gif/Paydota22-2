...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Public API endpoint for direct card charge
  app.post('/api/public/charge-card', async (req, res) => {
    try {
      const { txRef, cardNumber, cvv, expiryMonth, expiryYear, amount, currency, customer, authorization } = req.body;

      if (!txRef || !cardNumber || !cvv || !expiryMonth || !expiryYear) {
      ...
      const result = await flutterwaveService.chargeCard({
        txRef,
        cardNumber,
        cvv,
        expiryMonth,
        ...
        currency: currency || paymentLink.currency || 'USD',
        customer: {
          email: customer?.email || paymentLink.customerEmail,
          name: customer?.name || paymentLink.customerName || '',
          phonenumber: customer?.phonenumber || paymentLink.customerPhone || '',
        },
        authorization,
      });
      ...
      res.status(500).json({ message: error.message || 'Failed to charge card' });
    }
  });

  // Public API endpoint for OTP validation
  app.post('/api/public/validate-charge', async (req, res) => {
    try {
      ...
      if (result.status === 'success' && result.data?.status === 'successful' && txRef) {
        const paymentLink = await storage.getPaymentLinkByTxRef(txRef);
        
        let existingTransaction = await storage.getPaymentTransactionByTxRef(txRef);
        if (!existingTransaction) {
          existingTransaction = await storage.createPaymentTransaction({
            ...
            chargedAmount: result.data.charged_amount?.toString(),
            ...
            status: 'successful',
            cardNumber: result.data.card ? `${result.data.card.first_6digits}****${result.data.card.last_4digits}` : undefined,
          ...
          if (paymentLink?.userId) {
            ...
            await storage.createPendingBalance({
              ...
              releaseDate,
              description: `Payment from ${result.data.customer.email}`,
            });
          }
        }
      }
      ...
      res.status(500).json({ message: error.message || 'Failed to validate charge' });
    }
  });
